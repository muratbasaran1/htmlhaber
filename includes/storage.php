 ...
 * @return array<mixed>
 */
function loadJsonData(string $filename, array $default): array
{
    $path = getDataDirectory() . '/' . $filename;
 ...
 */
function saveJsonData(string $filename, array $data): void
{
    ensureDataDirectory();
    $path = getDataDirectory() . '/' . $filename;
    ...
    if ($json === false) {
        throw new RuntimeException('Veri JSON formatına dönüştürülemedi.');
    }

    if (file_put_contents($path, $json . PHP_EOL) === false) {
        throw new RuntimeException('Veri dosyaya kaydedilemedi: ' . $filename);
    }
}
...
function loadCategories(): array
{
    ...
    $categories = loadJsonData('categories.json', getDefaultCategories());
...
function loadArticles(): array
{
    ...
    $articles = loadJsonData('articles.json', getDefaultArticles());
...
{
    /** @var array<int, array<string, string>> $picks */
    $picks = loadJsonData('editorial_picks.json', getDefaultEditorialPicks());
...
{
    /** @var array<int, string> $ticker */
    $ticker = loadJsonData('live_ticker.json', getDefaultLiveTicker());
...
{
    /** @var array<int, array<string, string>> $subscribers */
    $subscribers = loadJsonData('subscribers.json', []);

    usort($subscribers, static function (array $a, array $b): int {
...
function saveArticles(array $articles): void
{
    ...
    saveJsonData('articles.json', array_values($articles));
}
...
{
    saveJsonData('editorial_picks.json', array_values($picks));
}
...
{
    $messages = array_values(array_filter(array_map('trim', $messages), static fn ($message) => $message !== ''));
    saveJsonData('live_ticker.json', $messages);
}

/**
 * @param array<int, array<string, string>> $subscribers
 */
function saveSubscribers(array $subscribers): void
{
    usort($subscribers, static function (array $a, array $b): int {
        return strcmp($b['subscribed_at'] ?? '', $a['subscribed_at'] ?? '');
    });

    saveJsonData('subscribers.json', array_values($subscribers));
}
...
function addSubscriber(string $email): array
{
    ...
    $record = [
        ...
        'subscribed_at' => (new DateTimeImmutable('now', new DateTimeZone('Europe/Istanbul')))
